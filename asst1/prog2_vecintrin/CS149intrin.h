...
// 如果向量lane是活动的(true)，则将寄存器设置为value，否则保留旧值
...
// 为了方便用户，返回一个所有lane初始化为value的向量寄存器
...
// otherwise keep the old value
// 如果向量lane是活动的(true)，则将向量寄存器src中的值复制到向量寄存器dest中，否则保留旧值
...
//  otherwise keep the old value
// 如果向量lane是活动的(true)，则将数组src中的值加载到向量寄存器dest中，否则保留旧值
